...
# Changed alias reporting from channel.reply to player tell. RCB / OrbitaL.
...
class aliases(minqlx.Plugin):
    ...
    def __init__(self):
        self.add_command("alias", self.cmd_alias, 3)
        self.add_command("clearaliases", self.cmd_clearaliases, 5)
        self.add_command("tomtec_versions", self.cmd_showversion)

        self.plugin_version = "1.7.1orb"
    ...
    def cmd_alias(self, player, msg, channel):
        ...
        player.tell("{}^7 has played on this server under the following names:".format(player_name))
        player.tell(" ^4*^7  {}".format("   \n ^4*^7  ".join(namelist)))

        player.tell("{}^7 has played on this server under the following Steam64 IDs:".format(player_name))
    ...
    def cmd_clearaliases(self, player, msg, channel):
        ...
        player.tell("All aliases for all players ({} players in total) were cleared.".format(len(players)))
